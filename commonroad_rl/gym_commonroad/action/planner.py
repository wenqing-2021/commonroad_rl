...
class PlanTrajectory:
    ...
    def convert_to_viz_trajectory(self, **kwargs):
        ...
        # only show the traj forward the ego vehicle
        nearest_idx = 0
        ego_pos = kwargs.get("ego_pos", None)
        if ego_pos is not None:
            x, y = ego_pos
            nearest_idx = np.argmin((self.cart_x - x) ** 2 + (self.cart_y - y) ** 2)
            nearest_idx = int(nearest_idx)
        trajectory_viz_params = TrajectoryParams(
            time_begin=nearest_idx,
        ...
        state_list = []
        for i in range(nearest_idx, len(self.cart_x)):
            state_list.append(
                STState(
                    time_step=i,
        ...
        return (
            Trajectory(initial_time_step=nearest_idx, state_list=state_list),
