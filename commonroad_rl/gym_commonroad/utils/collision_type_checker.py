...
def _current_collision_type_checking(
    ego_vehicle: Vehicle,
    observation_collector: ObservationCollector,
    scenario: Scenario,
    lane_change_time_threshold,
    ...
    for collision_object in collision_objects:
        ...
        for obstacle_id, obstacle_state in obstacle_state_dict.items():
            if isinstance(collision_shape, pycrcc.RectOBB) or isinstance(collision_shape, pycrcc.Circle):
                ...
                collision_shape_center = 0.5 * np.array(
                    [
                        collision_shape.min_x() + collision_shape.max_x(),
    ...
    merge_results = create_lanes(scenario.lanelet_network, merging_length=1000.0)
    ...
    for obstacle_id, (obstacle_state, collision_shape) in collision_obstacles.items():
        ...
        if obstacle.obstacle_type == ObstacleType.CAR:
            ...
            lanelets_contact_point = set(
                scenario.lanelet_network.find_lanelet_by_position([np.array(contact_point)])[0]
            )

            def find_lanelet_id_for_state(observation_collector, state, scenario):
                lanelet_polygons, lanelet_polygons_sg = observation_collector._get_lanelet_polygons(
            ...
            def find_lanelet_ids(state_list, time_steps):
                ...
                return set([find_lanelet_id_for_state(observation_collector, state, scenario) for state in state_list])
            ...
            obstacle_state_list = [
                obstacle.state_at_time(time_step)
                for time_step in range(obstacle.initial_state.time_step, current_step + 1)
            ]
...
def create_lanes(lanelet_network: LaneletNetwork, merging_length: float = 600.0):
    """
    ...
    for lanelet in start_lanelets:
        merged_lanelets, merge_jobs = Lanelet.all_lanelets_by_merging_successors_from_lanelet(
...
def _find_collision_point(
    ego_shape: Shape, obstacle_occupancy: Occupancy, collision_checker: pycrcc.CollisionChecker = None
):
