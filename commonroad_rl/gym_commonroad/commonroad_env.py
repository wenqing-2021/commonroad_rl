...
from reachable_set.reachable_set import ReachSet
...
class CommonroadEnv(gym.Env):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # reachable set
        self.use_reach_set = use_reach_set
    ...
    def reset(
        ...
        goal_x, goal_y = self.planning_problem.goal.state_list[0].position.center
        self.ego_action.reset(
            self.scenario.dt,
            local_ccosy=self.observation_collector.local_ccosy,
            goal_region=(goal_x, goal_y),
        )
        self.ilqr_trajectory = None
        info = dict()
        ...
        if self.use_reach_set:
            self.reach_set = ReachSet.build_reach_set(
                scenario=self.scenario,
                initial_state=self.ego_action.vehicle.state,
                clcs=self.observation_collector.local_ccosy,
            )
        else:
            self.reach_set = None
    ...
    def step(
        ...
        info = {
            ...
            "cost": cost,
            "position": [
                self.ego_action.vehicle.state.position[0],
                self.ego_action.vehicle.state.position[1],
            ],
            "control_output": [
                self.ego_action.controller.control_output[0],  # steer angle
                self.ego_action.controller.control_output[1],  # acc
            ],
            "obstalce_pos": obstalce_pos,
            "track_traj": [
                self.ego_action.current_trajectory.cart_x,
                self.ego_action.current_trajectory.cart_y,
                self.ego_action.current_trajectory.cart_v,
                self.ego_action.current_trajectory.cart_kappa,
                self.ego_action.current_trajectory.cart_a,
                self.ego_action.current_trajectory.frenet_s,
                self.ego_action.current_trajectory.frenet_l,
            ],
        }
    ...
    def render(self, mode: str = "human", **kwargs) -> None:
        ...
        if self.ego_action.current_trajectory is not None:
            traj_color = (250 / 255, 134 / 255, 0 / 255)
            (
                viz_trajecotry,
                viz_traj_params,
            ) = self.ego_action.current_trajectory.convert_to_viz_trajectory(
                traj_color=traj_color, ego_pos=self.ego_action.vehicle.state.position
            )
            self.cr_render.draw_trajectory(viz_trajecotry, viz_traj_params)
            traj_x = self.ego_action.current_trajectory.cart_x
            traj_y = self.ego_action.current_trajectory.cart_y
            x, y = self.ego_action.vehicle.state.position
            nearest_idx = int(np.argmin((traj_x - x) ** 2 + (traj_y - y) ** 2))
            radius = 0.25
            traj_point_drawparams = ShapeParams(
                facecolor=traj_color,
                edgecolor=traj_color,
                zorder=50,
            )
            for x, y in zip(traj_x[nearest_idx:], traj_y[nearest_idx:]):
                self.cr_render.draw_ellipse(
                    center=[x, y],
                    radius_x=radius,
                    radius_y=radius,
                    draw_params=traj_point_drawparams,
                )

        # show plan trajectory
        if self.ego_action.current_plan_trajectory is not None:
            traj_color = "red"
            (
                viz_trajecotry,
                viz_traj_params,
            ) = self.ego_action.current_plan_trajectory.convert_to_viz_trajectory(
                traj_color=traj_color, ego_pos=self.ego_action.vehicle.state.position
            )
            self.cr_render.draw_trajectory(viz_trajecotry, viz_traj_params)
        ...
        if self.reach_set is not None:
            ...
            last_step = self.reach_set.reach_interface.step_end
        ...
        if not self.render_configs["render_combine_frames"] or self.terminated:
            ...
            if self.render_configs["render_follow_ego"]:
                ...
                # follow ego
                x, y = self.ego_action.vehicle.state.position
                render_range = self.render_configs["render_range"]
        ...
        for render_dict, risk_result, planner_result, reachable_set_interface in zip(
            ...
            if render_dict is not None:
                ...
                if save_true:
                    ...
                    plt.ylabel("Y [m]", fontsize=label_size, fontname="Times New Roman")
                    plt.xticks(fontsize=label_size, fontname="Times New Roman")
                    plt.yticks(fontsize=label_size, fontname="Times New Roman")
                    plt.title(
                    ...
                    save_path = kwargs.get("save_path", "output/figs/render")
                    save_name = kwargs.get("save_name", "test")
