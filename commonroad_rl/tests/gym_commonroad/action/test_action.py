...
@pytest.mark.parametrize(
    ...
    [
        (np.array([1.0, 0.0]), np.array([-1.5, 0.5]), np.array([0.811, 0.584])),
        (np.array([1.0, 0.0]), np.array([-0.5, 1.0]), np.array([1.0, 0.0])),
        (np.array([0.7071, 0.7071]), np.array([0.5, 0.5]), np.array([0.973, -0.229])),
        (np.array([0.0, -1.0]), np.array([0.5, -0.5]), np.array([-0.923, 0.382])),
    ],
)
@unit_test
@functional
def test_curvi_rotation(vector: np.ndarray, pos: np.ndarray, rotated_vector: np.ndarray):
    curvi = CurvilinearCoordinateSystem(
        [np.array([-2, 0]), np.array([-1, 1]), np.array([0, 1]), np.array([1, 0]), np.array([0, -1])]
    )

    res = _rotate_to_curvi(vector, curvi, pos)
    assert np.allclose(res, rotated_vector, atol=1.0e-3)
...
def test_continuous_action_yaw_rate_rescale(vehicle_model, expected_value):
    ...
    action_configs = {"action_type": "continuous", "action_base": "acceleration"}  # acceleration; jerk
    ...
    with pytest.raises(AssertionError):
        action.rescale_action(np.array([1.0, 1.0]))
    action.reset(initial_state, dt=1.0)

    scaled_action = action.rescale_action(np.array([1.0, 1.0]))
