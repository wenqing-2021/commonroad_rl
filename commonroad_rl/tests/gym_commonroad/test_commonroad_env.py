...
pickle_xml_scenarios(input_dir=os.path.join(resource_path), output_dir=os.path.join(resource_path, "pickles"))
...
@pytest.mark.parametrize(
    ("num_of_checks", "test_env", "play"), [(15, False, False), (15, False, True), (15, True, False), (15, True, True)]
)
@module_test
@functional
...
@pytest.mark.parametrize(("reward_type"), [("hybrid_reward"), ("sparse_reward")])
@module_test
@functional
...
def test_reset_env_with_scenario():
    ...
    for i in range(1, 50):
        ...
        for obstacle in scenario.dynamic_obstacles:
            ...
            next_obstacle = DynamicObstacle(
                obstacle.obstacle_id, obstacle.obstacle_type, obstacle.obstacle_shape, obstacle_state, prediction=None
            )
        ...
        ego_next_state.slip_angle = 0.0

        observations_reset = env_reset.reset(
            scenario=next_scenario,
            planning_problem=PlanningProblem(
                planning_problem.planning_problem_id, ego_next_state, planning_problem.goal
            ),
        )
        ...
        if collision_reset != collision_step:
            ...
            co_reset_obstacle = pycrcc.RectOBB(
                obstacle_reset.obstacle_shape.length / 2,
            ...
            co_step_obstacle = pycrcc.RectOBB(
                obstacle_step.obstacle_shape.length / 2,
        ...
        assert (
            collision_reset
            == collision_step
            == env_step.observation_dict["is_collision"]
            == env_reset.observation_dict["is_collision"]
        ), (
            f"step={i} "
        ...
        observations_reset[obs_distance_goal_long_advance_idx] = abs(obs_distance_goal_long) - abs(
            observations_reset[obs_distance_goal_long_idx]
        )
        observations_reset[obs_distance_goal_lat_advance_idx] = abs(obs_distance_goal_lat) - abs(
            observations_reset[obs_distance_goal_lat_idx]
        )
        ...
        assert np.allclose(observations_reset, observations_step), (
            f"{env_reset.observation_dict['is_collision']}, "
        ...
        if not np.allclose(observations_reset, observations_step):
            print(
                f"ego_state: env_reset={env_reset.ego_action.vehicle.state}, env_step={env_step.ego_action.vehicle.state}"
            )
            keys = []
            ...
            for idx in idxs[0]:
                ...
                print(
                    f"{observation_keys[idx]}: observation_reset={observations_reset[idx]}, observation_step={observations_step[idx]}"
                )
            raise ValueError


@pytest.mark.parametrize(("reward_type"), [("sparse_reward"), ("hybrid_reward")])
@module_test
@functional
