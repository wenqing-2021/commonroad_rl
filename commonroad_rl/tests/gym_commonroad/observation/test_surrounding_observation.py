...
@pytest.mark.parametrize(
    ("ego_state", "observe_lane_rect_surrounding", "observe_lane_circ_surrounding", "p_rel_expected", "v_rel_expected"),
    [
        (
            CustomState(
                **{
                    "time_step": 50,
                    "position": np.array([127.50756356637483, -50.69294785562317]),
        ...
        (
            ...
            True,
            False,
            [50.1223503, 17.3700263, 50.1223503, 50.1223503, 50.1223503, 50.1223503],
        ...
        (
            ...
            False,
            True,
            [50.0, 18.20347091, 50.0, 50.0, 47.40590347, 50.0],
        ...
        (
            ...
            False,
            True,
            [50.0, 18.05391643, 50.0, 50.0, 47.63507943, 50.0],
        ...
        (
            ...
            False,
            True,
            [50.0, 17.92129415, 50.0, 50.0, 47.8514526, 50.0],
...
@module_test
@functional
def test_get_surrounding_obstacles_lane_based(
    ego_state, observe_lane_rect_surrounding, observe_lane_circ_surrounding, p_rel_expected, v_rel_expected
):
    """
    Test for
    ...
    surrounding_observation = SurroundingObservation(
        configs={
            "reward_configs": {"hybrid_reward": {"reward_safe_distance_coef": -1.0}},
            "surrounding_configs": {
                "observe_lane_rect_surrounding": observe_lane_rect_surrounding,
    ...
    ego_lanelet_ids = ObservationCollector.sorted_lanelets_by_state(
        scenario_1, ego_state, lanelet_polygons, lanelet_polygons_sg
    )
    ...
    local_ccosy, _ = ObservationCollector.get_local_curvi_cosy(
        scenario_1.lanelet_network, ego_lanelet_id, None, max_lane_merge_range=1000.0
    )
    collision_checker = create_collision_checker(scenario_1)
    surrounding_observation.observe(
        scenario_1,
        ego_vehicle,
        ego_state.time_step,
        connected_lanelet_dict,
        ego_lanelet,
        collision_checker=collision_checker,
        local_ccosy=local_ccosy,
    )
    ...
    assert np.allclose(
        np.array(p_rel), np.array(p_rel_expected), atol=1e-2
    ...
    assert np.allclose(
        np.array(v_rel), np.array(v_rel_expected), atol=1e-2
...
def test_obstacle_detection_lane_based():
    ...
    local_ccosy, _ = ObservationCollector.get_local_curvi_cosy(
        scenario.lanelet_network, ego_lanelet_id, None, max_lane_merge_range=1000.0
    )
    ...
    surrounding_observation.observe(
        ...
        ego_vehicle,
        ego_state.time_step,
        connected_lanelet_dict={1: {1}, 2: {2}, 3: {3}},
        ego_lanelet=ego_lanelet,
        collision_checker=collision_checker,
        local_ccosy=local_ccosy,
    )
...
@pytest.mark.parametrize(
    ("ego_state", "prev_rel_pos", "p_rel_expected", "p_rel_rate_expected"),
    [
        (
            CustomState(
                **{
...
@unit_test
@functional
def test_get_surrounding_obstacles_lidar_circle(
    ego_state,
    prev_rel_pos,
    p_rel_expected,
    p_rel_rate_expected,
    ...
    surrounding_observation = SurroundingObservation(
        configs={
            ...
            "surrounding_configs": {
                ...
                "observe_lane_circ_surrounding": False,
                "observe_intersection_distances": False,
                "observe_intersection_velocities": False,
                "dummy_dist_intersection": 50,
                "observe_ego_distance_intersection": False,
...
@pytest.mark.parametrize(
    ("obstacle_shapes", "ego_state", "actual_detection_points"),
    [
        (
...
@unit_test
@functional
def test_get_distances_lidar_based(
    obstacle_shapes: List[Polygon], ego_state: State, actual_detection_points: np.ndarray
):
    configs = {
        "reward_configs": {"hybrid_reward": {"reward_safe_distance_coef": -1.0}},
        "surrounding_configs": {
            "lidar_circle_num_beams": 4,
            "lidar_sensor_radius": 10.0,
            "observe_lidar_circle_surrounding": True,
        },
    }
    ...
    s_obs.max_obs_dist = 100.0
    s_obs._ego_state = ego_state
    obstacle_distances, _ = s_obs._get_obstacles_with_surrounding_beams(
        obstacle_shapes, detected_obstacles, surrounding_beams_ego_vehicle
    )
    s_obs._current_time_step = 0
    dists, dist_rates, detection_points = s_obs._get_distances_lidar_based(
...
@unit_test
@functional
def test_get_nearby_lanelet_id(
    connected_ego: set, connected_successor: set, connected_predecessor: set, connected_left: set, connected_right: set
):
    ...
    lanelet_dict, all_lanelets_set = SurroundingObservation.get_nearby_lanelet_id(
...
@pytest.mark.parametrize(
    ("ego_state", "ego_lanelet_id", "desired_p_rel", "desired_v_rel"),
    [
        (
            CustomState(**{"time_step": 0, "position": np.array([21, 3.8]), "orientation": 0, "velocity": 0}),
            30626,
            [100.0, 100.0, 100.0, 100.0, 0.38061144, 100.0],
            [0.0, 0.0, 0.0, 0.0, 35.0, 0.0],
        ),
        (
            CustomState(**{"time_step": 0, "position": np.array([-39, -4]), "orientation": 0, "velocity": 0}),
            30622,
            [100.0, 100.0, 100.0, 100.0, 65.750643, 100.0],
...
@unit_test
@functional
def test_get_rel_v_p_lane_based(
    ego_state: State, ego_lanelet_id: int, desired_p_rel: List[float], desired_v_rel: List[float]
):
    """
    test for get_rel_v_p_lane_based method of SurroundingObservations
    """
    configs = {"reward_configs": {"hybrid_reward": {"reward_safe_distance_coef": -1.0}}, "surrounding_configs": {}}
    obs = SurroundingObservation(configs)
    obs.max_obs_dist = 100.0
    obs._local_ccosy = ObservationCollector.get_local_curvi_cosy(
        scenario_2.lanelet_network, ego_lanelet_id, None, max_lane_merge_range=5000.0
    )[0]
    ...
    lanelet_dict = SurroundingObservation.get_nearby_lanelet_id(
        connected_lanelet_dict, scenario_2.lanelet_network.find_lanelet_by_id(ego_lanelet_id)
    )[0]

    v_rel, p_rel, _, _, ego_vehicle_lat_position = obs._get_rel_v_p_lane_based(
        obstacles_lanelet_ids, obstacle_states, lanelet_dict, scenario_2.obstacles
    )
...
@pytest.mark.parametrize(
    ("obs_state", "ego_state", "follow", "desired_v_rel", "desired_p_rel"),
    [
        (
            CustomState(**{"time_step": 2, "position": np.array([0, -3]), "orientation": 0, "velocity": 0}),
            CustomState(**{"time_step": 2, "position": np.array([0, -3]), "orientation": 0, "velocity": 0}),
        ...
        (
            CustomState(**{"time_step": 2, "position": np.array([0, -3]), "orientation": 0, "velocity": 0}),
            CustomState(**{"time_step": 2, "position": np.array([0, -320]), "orientation": 0, "velocity": 0}),
            False,
            0,
            0,
        ),
        (
            CustomState(**{"time_step": 2, "position": np.array([0, -3]), "orientation": 0, "velocity": 0}),
            CustomState(**{"time_step": 2, "position": np.array([0, -20]), "orientation": 0, "velocity": 4}),
            True,
            4,
            17.0,
        ),
...
@unit_test
@functional
def test_get_rel_v_p_follow_leading(
    obs_state: State, ego_state: State, follow: bool, desired_v_rel: float, desired_p_rel: float
):
...
@pytest.mark.parametrize(
    ("ego_state", "obstacle_shapes", "desired_distances", "lidar_circle_num_beams"),
    [
        (
            CustomState(**{"time_step": 2, "position": np.array([0, -3]), "orientation": 0, "velocity": 0}),
            [
                PolygonShapely(np.array([[0, 0], [1, 1], [0, 1]])),
            ],
            np.array([100.0, 3.0, 100.0, 100.0]),
            4,
        ),
        (
            CustomState(**{"time_step": 2, "position": np.array([0, -3]), "orientation": 0, "velocity": 0}),
            [
                PolygonShapely(np.array([[0, 0], [1, 1], [0, 1]])),
                PolygonShapely(np.array([[-3, 0], [-2, 1], [-3, 1]])),
                PolygonShapely(np.array([[4, -4], [5, -4], [5, -2], [4, -2]])),
            ],
            np.array([4.0, 100, 3.0, 3 * np.sqrt(2), 100, 100, 100.0, 100.0]),
            8,
        ),
        (
            CustomState(**{"time_step": 2, "position": np.array([0, -3]), "orientation": 0, "velocity": 0}),
            ...
            np.array([4.0, 100, 3.0, 3 * np.sqrt(2), 100, 100, 4.0, 100.0]),
            8,
        ),
    ],
)
@unit_test
@functional
def test_get_obstacles_with_surrounding_beams(
    ego_state: State, obstacle_shapes: List[Polygon], desired_distances: np.ndarray, lidar_circle_num_beams: int
):
    """
    ...
    obs._ego_state = ego_state
    detected_obstacles = [None] * len(obstacle_shapes)
    obstacle_distances, _ = obs._get_obstacles_with_surrounding_beams(
        obstacle_shapes, detected_obstacles, surrounding_beams_ego_vehicle
    )
...
@pytest.mark.parametrize(
    ("obstacle_state", "obstacle_lanelet_ids", "desired_id"),
    [
        (CustomState(**{"time_step": 2, "position": np.array([0, -3]), "orientation": 0, "velocity": 0}), [0], 0),
        (
            CustomState(**{"time_step": 2, "position": np.array([-19, -2]), "orientation": 0, "velocity": 0}),
            [30622, 30624],
            30624,
        ),
        (
...
@pytest.mark.parametrize(
    ("lanelet_ids", "states", "all_lanelets_set", "filtered_lanelet_ids"),
    [
...
@unit_test
@functional
def test_filter_obstacles_in_adj_lanelet(
    lanelet_ids: List[int], states: List[State], all_lanelets_set: set, filtered_lanelet_ids: List[int]
):
    obstacles = [None] * len(states)
    obstacle_lanelet, obstacle_state, _ = SurroundingObservation._filter_obstacles_in_adj_lanelet(
        lanelet_ids, states, obstacles, all_lanelets_set
    )
...
@pytest.mark.parametrize(
    ("surrounding_area", "current_timestep", "desired_lanelet_ids"),
    [
        ...
        (
            create_collision_object(Polygon(np.array([[-39, 4.87], [387, 9.27], [387, 5.74], [-39.3, 1.34]]))),
            0,
            [30626],
        ),
    ],
...
@unit_test
@functional
def test_get_obstacles_in_surrounding_area(
    surrounding_area: pycrcc.Shape, current_timestep: int, desired_lanelet_ids: List[int]
):
    """
    current_timestep
    scenario
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (7, np.array([-13.32364251, 6.12420627])),
    ],
)  # number before change -16.92928943, 6.16155603
@module_test
@functional
def test_get_ego_intersection_distance(obs_num, expected_observation):
    ...
    # find ego lanelet here because multiple observations need it
    ego_lanelet_ids = ObservationCollector.sorted_lanelets_by_state(
        scenario_3, state, lanelet_polygons_3, lanelet_polygons_sg_3
    )
    ego_lanelet_id = ego_lanelet_ids[0]
    ...
    local_ccosy, _ = ObservationCollector.get_local_curvi_cosy(
        scenario_3.lanelet_network, ego_lanelet_id, None, max_lane_merge_range=1000.0
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            0,
            np.array([-18.88140868, -16.83738288, -14.17917091, 3.22666699, 8.97399696, 14.7696437]),
            np.array([16.2605, 12.01397, 7.23233, 0.0, 0.0, 0.0]),
        ),
        (
            1,
            np.array([-19.5329787, -17.32711044, -14.44436202, 3.22666699, 8.97399696, 14.7696437]),
            np.array([16.27618, 12.00737, 7.23564, 0.0, 0.0, 0.0]),
        ),
        (50, np.array([3.22666699, 8.97399696, 14.7696437, 50.0, 50.0, 50.0]), np.array([0, 0, 0, 0, 0, 0])),
        (
            100,
            np.array([-12.49805328, 3.22666699, 8.97399696, 14.7696437, 16.30804512, 50.0]),
            np.array([11.31547, 0.0, 0.0, 0.0, 10.80843, 0.0]),
        ),
    ],
)
@module_test
...
def test_get_intersection_distances(time_step, expected_distances, expected_velocities):
    ...
    surrounding_observation = SurroundingObservation(
        configs={
            ...
            "surrounding_configs": {
                ...
                "dummy_dist": 60.0,
                "observe_is_collision": False,
                "observe_intersection_distances": True,
                "observe_intersection_velocities": True,
                "dummy_dist_intersection": 50,
                "observe_ego_distance_intersection": False,
            },
        }
    ...
    state = CustomState(
        **{
            "time_step": time_step,
            "position": np.array([40.09902, -16.25447]),
            "orientation": 5.4828199,
            "velocity": 5.77779,
        }
    )
    ego_vehicle.reset(state, dt=0.04)
...
@pytest.mark.parametrize(
    ("obs", "expected_lights"),
    [(scenario_1.dynamic_obstacles, 10 * [0]), (scenario_2.dynamic_obstacles, 2 * [0]), ([None, None], [0, 0])],
)
@unit_test
@functional
...
def test_continuous_collision_checking(preprocess):
    ...
    state_list = [
        CustomState(position=np.array([10.0, 0.0]), velocity=10.0, orientation=0.0, time_step=1),
        CustomState(position=np.array([20.0, 0.0]), velocity=10.0, orientation=0.0, time_step=2),
    ]
    dynamic_obstacle = DynamicObstacle(
        scenario.generate_object_id(),
        ObstacleType.CAR,
        obstacle_shape,
        CustomState(position=np.array([0.0, 0.0]), velocity=10.0, orientation=0.0, time_step=0),
    ...
    initial_state = CustomState(
        position=np.array([5.0, -5.0]), velocity=0.0, velocity_y=10.0, orientation=np.pi / 2, time_step=0
    )
    dt = 1.0
    ...
    next_state = CustomState(
        position=np.array([5.0, 5.0]), velocity=0.0, velocity_y=10.0, orientation=np.pi / 2, time_step=1
    )
