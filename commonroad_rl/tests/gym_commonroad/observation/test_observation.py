...
@unit_test
@functional
def test_related_lanelets_by_state(
    lanelet_vertices: List[Tuple[List[List[float]]]],
...
@pytest.mark.parametrize(
    ("lanelets", "cartesian_coordinates", "expected_curvilinear_coordinates", "desired_merged_lanelet"),
    [
        (
            [
                Lanelet(
                    lanelet_id=0,
                    left_vertices=np.array([[-1, 1], [1, 1]]),
                    center_vertices=np.array([[-1, 0], [1, 0]]),
                    right_vertices=np.array([[-1, -1], [1, -1]]),
                    adjacent_right_same_direction=1,
                ),
                Lanelet(
                    lanelet_id=1,
                    left_vertices=np.array([[-1, -1], [1, -1]]),
                    center_vertices=np.array([[-1, -2], [1, -2]]),
                    right_vertices=np.array([[-1, -3], [1, -3]]),
                    adjacent_left_same_direction=0,
                ),
            ],
            np.array([0, 0]),
            np.array([1, 2]),
        ...
        (
            [
                ...
                Lanelet(
                    lanelet_id=1,
                    left_vertices=np.array([[1, 1], [3, 1]]),
        ...
        (
            [
                ...
                Lanelet(
                    lanelet_id=3,
                    left_vertices=np.array([[-1, -3], [1, -3]]),
                    center_vertices=np.array([[-1, -4], [1, -4]]),
                    right_vertices=np.array([[-1, -5], [1, -5]]),
                    adjacent_left_same_direction=2,
                ),
            ],
            np.array([2, 4]),
            np.array([3, 4]),
...
@unit_test
@functional
def test_get_local_curvi_cosy(
    lanelets: List[Lanelet],
    cartesian_coordinates: np.ndarray,
    expected_curvilinear_coordinates: np.ndarray,
    desired_merged_lanelet: int,
):
    ...
    curvi_cosy, ref_merged_lanelet = ObservationCollector.get_local_curvi_cosy(
        lanelet_network, ego_vehicle_lanelet_id, ref_path_dict, max_lane_merge_range
    )
