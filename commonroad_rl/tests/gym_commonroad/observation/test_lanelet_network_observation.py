...
def prepare_for_test():
    ...
    vehicle = ContinuousAction(
        {"vehicle_type": vehicle_type, "vehicle_model": vehicle_model}, {"action_base": "acceleration"}
    )
...
@functional
@unit_test
def test_get_relative_future_goal_offsets(
    goal_region: GoalRegion, ego_state: State, sampling_points: List[int], static: bool, expected_offset
):
    """unittest for LaneletNetworkObservation._get_relative_future_goal_offsets method"""
    configs = {"lanelet_configs": {}}

    planning_problem = PlanningProblem(planning_problem_id=0, initial_state=ego_state, goal_region=goal_region)
...
def test_is_off_road_end():
    vehicle_action.reset(
        CustomState(position=np.array([387.5, 5.0]), orientation=0.0, velocity=0.0, time_step=0), dt=0.1
    )
...
def test_end_of_road_termination():
    ...
    env.ego_action.reset(
        CustomState(position=np.array([387.5, 5.0]), orientation=0.0, velocity=1.0, time_step=0), dt=0.1
    )
...
def test_get_distance_to_marker_and_road_edge(action, steps, expected_output):
    ...
    ego_lanelet_ids = ObservationCollector.sorted_lanelets_by_state(
        scenario, vehicle_action.vehicle.state, lanelet_polygons, lanelet_polygons_sg
    )
    ...
    result = np.hstack(
        (
            lanelet_observation.observation_dict["left_marker_distance"],
...
@pytest.mark.parametrize(
    ...
    [
        (0, LineString([[0, 1], [3, 4]]), LineString([[112, 3], [23, 4]])),
        (3, LineString([[0, 1], [3, 4]]), LineString([[112, 3], [23, 4]])),
        (34, LineString([[0, 1], [3, 4], [2, 34]]), LineString([[112, 3], [23, 4], [2314, 34]])),
    ],
)
@functional
@unit_test
...
@pytest.mark.parametrize(
    ...
    [
        (Point(1, -1), LineString([[0, 0], [1, 0], [1, 1]]), 1),
        (Point(1, -1), LineString([[0, 0], [1, 0], [1, -1]]), 0),
        (Point(1, 2), LineString([[0, 0], [1, 0], [1, 1]]), 1),
    ],
)
@functional
@unit_test
...
@pytest.mark.parametrize(
    (
        ...
        "left_edge",
        "right_edge",
        "desired_left_marker_distance",
        "desired_right_marker_distance",
        "desired_left_edge_distance",
        "desired_right_edge_distance",
    ),
    [
        (
            np.array([0, 0]),
...
@functional
@unit_test
def test_get_distance_to_marker_and_road_edge(
    position: np.ndarray,
    left_marker: LineString,
    right_marker: LineString,
    left_edge: LineString,
    right_edge: LineString,
    desired_left_marker_distance: float,
    desired_right_marker_distance: float,
    desired_left_edge_distance: float,
    desired_right_edge_distance: float,
):
    ...
    (
        ...
        ego_state, left_marker, right_marker, left_edge, right_edge
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (np.array([0.0, 0.0]), np.array([[0, 0], [1, 1], [2, 2], [3, 3], [4, 4]]), 0.0),
    ],
)
@functional
@unit_test
def test_get_lane_curvature(position, curve, expected_output):
    ...
    )  # import here cause otherwise it's overwritten by pycrccosy.CurvilinearCoordinateSystem
