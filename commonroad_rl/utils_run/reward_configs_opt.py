...
def optimize_reward_configs(
    ...
    def objective_reward_configs(trial):
        ...
        sampled_reward_configs = sample_commonroad_reward_configs(
            trial, sampling_setting, trial_dict, based_on_dict, log_path, LOGGER, guided
        )
    ...
    if guided:  # guided optimization
        ...
        while not updated:
            if compare_trial == -1:
                ...
                previous_rewards_path = os.path.join(
                    os.path.join(log_path, previous_trial), "sampled_reward_configurations.yml"
                )
                ...
                if based_on_dict[previous_trial] != None:  # second trial before current is found
                    ...
                    second_to_prev_rewards_path = os.path.join(
                        os.path.join(log_path, second_to_prev_trial), "sampled_reward_configurations.yml"
                    )
                    ...
                    for failure, value in GUIDED.items():  # for each type of failure: off_road etc.
                        for key, subkeys in value.items():  # for each reward type
                            for subkey in subkeys:  # for each reward
                                ...
                                change_in_reward_value = (
                                    previous_rewards[key][subkey] - second_to_prev_rewards[key][subkey] + 0.000001
                                )  # change reward
                                change_in_failure = abs(
                                    SIGN_OF_IMPROVEMENT[subkey][1] - second_to_prev_rates[failure]
                                ) - abs(
                                    SIGN_OF_IMPROVEMENT[subkey][1] - previous_rates[failure]
                                ...
                                reward_configs[key][subkey] = float(
                                    previous_rewards[key][subkey]
                                    + int(SIGN_OF_IMPROVEMENT[subkey][0])
                                    * (-change_in_failure / np.sign(change_in_reward_value) + scaling * 3)
                                )
                                logger.info(
                                    f"reward {subkey} set to {float(previous_rewards[key][subkey] + int(SIGN_OF_IMPROVEMENT[subkey][0]) * (- change_in_failure / np.sign(change_in_reward_value) + scaling * 3))}"
                                )
                                logger.debug(f" failure {failure}")
                                logger.debug(f"rates {previous_rates[failure]}")
                                logger.debug(
                                    f"adapted rates {abs(SIGN_OF_IMPROVEMENT[subkey][1] - previous_rates[failure])}"
                                )
                                logger.debug(
                                    f"new rewards {float(previous_rewards[key][subkey] + int(SIGN_OF_IMPROVEMENT[subkey][0]) * (- change_in_failure / np.sign(change_in_reward_value) + scaling * 3))}"
                                )
                                logger.debug(
                                    f"change {float(int(SIGN_OF_IMPROVEMENT[subkey][0]) * (- change_in_failure / np.sign(change_in_reward_value) + scaling * 3))}"
                                )
                        ...
                        for key, subkeys in value.items():
                            for subkey in subkeys:
                                ...
                                reward_configs[key][subkey] = float(
                                    previous_rewards[key][subkey]
                                    + int(SIGN_OF_IMPROVEMENT[subkey][0])
                                    * abs(SIGN_OF_IMPROVEMENT[subkey][1] - previous_rates[failure])
                                    * 3
                                )
                                logger.info(
                                    f"reward {subkey} set to {float(previous_rewards[key][subkey] + int(SIGN_OF_IMPROVEMENT[subkey][0]) * abs(SIGN_OF_IMPROVEMENT[subkey][1] - previous_rates[failure]) * 3)}"
                                )
                                logger.debug(f" failure {failure}")
                                logger.debug(f"rates {previous_rates[failure]}")
                                logger.debug(
                                    f"new rewards {float(previous_rewards[key][subkey] + int(SIGN_OF_IMPROVEMENT[subkey][0]) * abs(SIGN_OF_IMPROVEMENT[subkey][1] - previous_rates[failure]) * 3)}"
                                )
                #  updated allows to stop looking for a comparisson trial
                updated = True
            else:
        ...
        for failure, value in GUIDED.items():  # for each type of failure: off_road etc.
            for key, subkeys in value.items():  # for each reward type
                for subkey in subkeys:
                    reward_configs[key][subkey] = (
                        reward_configs[key][subkey] / total_reward_of_optimized
                    ) * SUM_OF_REWARD_VALUES
